...
namespace svm {
	...
	void init_thread_dataset(int nepochs){
    ...
    void Construct_Distribution_Distributed(void) {
    ...
    void Generate_Sample_Sequence_Distributed(int epoch) {
          ...
          for(int i=0;i<nthreads;i++){
              ...
              for(int count=0;count<trunk_len*epoch;count++){
                  if(use_prob){
  ...
  void Hogwild<Model, Params, Exec>::RunExperiment(int nepochs, TrainScan &trscan) {
	...
	Construct_Distribution_Distributed();
	Generate_Sample_Sequence_Distributed(nepochs);
